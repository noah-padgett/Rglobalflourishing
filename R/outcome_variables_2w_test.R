...
#' }
#' @export
get_outcome_better_name <- function(var, include.name = TRUE, include.wave = FALSE) {
  var0 = var
  var.wave = ""
  for(i in 1:5){
  ...
  if(include.name){
    ...
    var.tmp = paste0(var.tmp, " (Wave ", stringr::str_sub(var.wave, start=-1, end=-1),")")
  }
  var.tmp
}
...
recode_labels <- function(x, var) {
  ...
  switch(var,
         ID = x,
         COUNTRY = case_when(x == 1 ~ "1. Argentina", x == 2 ~ "2. Australia", x == 3 ~ "3.
...
United Kingdom", x == 22 ~ "22. United States", x == 23 ~ "23. Sweden", x == 24
         ...
         WAVE = x,
         MODE_RECRUIT = case_when(x == 1 ~ "1. CAPI", x == 2 ~ "2. CATI", x == 3 ~ "3. CAWI",
                                     .default =
                                       "(Missing)"
         ),
         MODE_ANNUAL = case_when(x == 1 ~ "1. CAPI", x == 2 ~ "2. CATI", x == 3 ~ "3. CAWI",
                                    .default =
                                      "(Missing)"
         ),
         RECRUIT_TYPE = case_when(x == 1 ~ "1. Separate Recruitment and Annual Surveys", x == 2 ~ "2.
Combined Recruitment and Annual Surveys", .default = "(Missing)"),
         DOI_RECRUIT = x,
         DOI_ANNUAL = x,
         ABUSED = case_when(x == 1 ~ "1. Yes", x == 2 ~ "2. No", .default = "(Missing)"),
         AFTER_DEATH = case_when(x == 1 ~ "1. Yes", x == 2 ~ "2. No", x == 3 ~ "3. Unsure",
                                    .default =
                                      "(Missing)"
         ),
         AGE = x,
         APPROVE_GOVT = case_when(x == 1 ~ "1. Strongly approve", x == 2 ~ "2. Somewhat approve", x == 3
                                     ~ "3. Neither approve nor disapprove", x == 4 ~ "4. Somewhat disapprove", x == 5
                                     ~ "5. Strongly disapprove", .default = "(Missing)"),
         ATTEND_SVCS = case_when(x == 1 ~ "1. More than once a week", x == 2 ~ "2. Once a week", x ==
                                      3 ~ "3. One to three times a month", x == 4 ~ "4. A few times a year", x == 5 ~
                                      "5. Never", .default = "(Missing)"),
         BELIEVE_GOD = case_when(x == 1 ~ "1. One God", x == 2 ~ "2. More than one god", x == 3 ~
                                      "3. An impersonal spiritual force", x == 4 ~ "4. None of these", x == 5 ~ "5.
Unsure", .default = "(Missing)"),
         BELONGING = x,
         BODILY_PAIN = case_when(x == 1 ~ "1. A lot", x == 2 ~ "2. Some", x == 3 ~ "3. Not very much",
                                    x == 4 ~ "4. None at all",
                                    .default = "(Missing)"
         ),
         BORN_COUNTRY = case_when(x == 1 ~ "1. Born in this country", x == 2 ~ "2. Born in another
country", .default = "(Missing)"),
         CAPABLE = case_when(x == 1 ~ "1. Always", x == 2 ~ "2. Often", x == 3 ~ "3. Rarely", x ==
         ...
         CNTRY_REL_JUD = x,
         CNTRY_REL_SHI = x,
         COMFORT_REL = case_when(x == 1 ~ "1. Agree", x == 2 ~ "2. Disagree", x == 3 ~ "3. Not
relevant", x == 4 ~ "4. Unsure", .default = "(Missing)"),
         CONNECTED_REL = case_when(x == 1 ~ "1. Always", x == 2 ~ "2. Often", x == 3 ~ "3. Rarely", x ==
                                        4 ~ "4. Never", .default = "(Missing)"),
         CONTENT = x,
         CONTROL_WORRY = case_when(x == 1 ~ "1. Nearly every day", x == 2 ~ "2. More than half the days",
                                      x == 3 ~ "3. Several days", x == 4 ~ "4. Not at all",
                                      .default = "(Missing)"
         ),
         COVID_DEATH = case_when(x == 1 ~ "1. Yes", x == 2 ~ "2. No", .default = "(Missing)"),
         CRITICAL = case_when(x == 1 ~ "1. Agree", x == 2 ~ "2. Disagree", x == 3 ~ "3. Not
relevant", x == 4 ~ "4. Unsure", .default = "(Missing)"),
         DAYS_EXERCISE = case_when(x == 0 ~ "0. 0 days", x == 1 ~ "1. 1 day", x == 2 ~ "2. 2 days", x
                                      == 3 ~ "3. 3 days", x == 4 ~ "4. 4 days", x == 5 ~ "5. 5 days", x == 6 ~ "6. 6
days", x == 7 ~ "7. 7 days/Every day", .default = "(Missing)"),
         DEPRESSED = case_when(x == 1 ~ "1. Nearly every day", x == 2 ~ "2. More than half the days",
                                  x == 3 ~ "3. Several days", x == 4 ~ "4. Not at all",
                                  .default = "(Missing)"
         ),
         DISCRIMINATED = case_when(x == 1 ~ "1. Always", x == 2 ~ "2. Often", x == 3 ~ "3. Rarely", x ==
                                        4 ~ "4. Never", .default = "(Missing)"),
         DONATED = case_when(x == 1 ~ "1. Yes", x == 2 ~ "2. No", .default = "(Missing)"),
         DRINKS = x,
         EDUCATION = case_when(x == 100 ~ "100. Argentina: (No formal education)", x == 101 ~ "101.
...
University Postgraduate Degree (e.g., MBA, Ph. D)",
         ...
         EMPLOYMENT = case_when(x == 1 ~ "1. Employed for an employer", x == 2 ~ "2. Self-employed",
...
these/Other",
         ...
         EXPECT_GOOD = x,
         EXPENSES = x,
         FATHER_LOVED = case_when(x == 1 ~ "1. Yes", x == 2 ~ "2. No", x == 97 ~ "97. (Does not
apply)", .default = "(Missing)"),
         FATHER_RELATN = case_when(x == 1 ~ "1. Very good", x == 2 ~ "2. Somewhat good", x == 3
                                      ~ "3. Somewhat bad", x == 4 ~ "4. Very bad", x == 97 ~ "97. (Does not
apply)", .default = "(Missing)"),
         FEEL_ANXIOUS = case_when(x == 1 ~ "1. Nearly every day", x == 2 ~ "2. More than half the days",
                                     x == 3 ~ "3. Several days", x == 4 ~ "4. Not at all",
                                     .default = "(Missing)"
         ),
         FORGIVE = case_when(x == 1 ~ "1. Always", x == 2 ~ "2. Often", x == 3 ~ "3. Rarely", x ==
                                  4 ~ "4. Never", .default = "(Missing)"),
         FREEDOM = x,
         GENDER = case_when(x == 1 ~ "1. Male", x == 2 ~ "2. Female", x == 3 ~ "3. Other", x == 4
                               ~ "4. Prefer not to answer", .default = "(Missing)"),
         GIVE_UP = x,
         GOD_PUNISH = case_when(x == 1 ~ "1. Agree", x == 2 ~ "2. Disagree", x == 3 ~ "3. Not
relevant", x == 4 ~ "4. Unsure", .default = "(Missing)"),
         GRATEFUL = x,
         GROUP_NOT_REL = case_when(x == 1 ~ "1. More than once a week", x == 2 ~ "2. Once a week", x ==
                                        3 ~ "3. One to three times a month", x == 4 ~ "4. A few times a year", x == 5 ~
                                        "5. Never", .default = "(Missing)"),
         HAPPY = x,
         HEALTH_GROWUP = case_when(x == 1 ~ "1. Excellent", x == 2 ~ "2. Very good", x == 3 ~ "3. Good",
         ...
         HOPE_FUTURE = x,
         INCOME = case_when(x == 101 ~ "101. Argentina: 1,000 pesos or less", x == 102 ~ "102.
...
household income)",
         ...
         INCOME_12YRS = case_when(x == 1 ~ "1. Lived comfortably", x == 2 ~ "2. Got by", x == 3 ~
                                       "3. Found it difficult", x == 4 ~ "4. Found it very difficult",
                                     .default =
                                       "(Missing)"
         ),
         INCOME_DIFF = case_when(x == 1 ~ "1. Strongly agree", x == 2 ~ "2. Somewhat agree", x == 3 ~
                                      "3. Neither agree nor disagree", x == 4 ~ "4. Somewhat disagree", x == 5 ~ "5.
Strongly disagree", .default = "(Missing)"),
         INCOME_FEELINGS = case_when(x == 1 ~ "1. Living comfortably on present income", x == 2 ~ "2.
...
income", x == 4 ~ "4. Finding it very difficult on present income",
         ...
         INTEREST = case_when(x == 1 ~ "1. Nearly every day", x == 2 ~ "2. More than half the days",
                                 x == 3 ~ "3. Several days", x == 4 ~ "4. Not at all",
                                 .default = "(Missing)"
         ),
         LIFE_APPROACH = case_when(x == 1 ~ "1. Agree", x == 2 ~ "2. Disagree", x == 3 ~ "3. Not
relevant", x == 4 ~ "4. Unsure", .default = "(Missing)"),
         LIFE_BALANCE = case_when(x == 1 ~ "1. Always", x == 2 ~ "2. Often", x == 3 ~ "3. Rarely", x ==
                                       4 ~ "4. Never", .default = "(Missing)"),
         LIFE_PURPOSE = x,
         LIFE_SAT = x,
         LONELY = x,
         LOVED_BY_GOD = case_when(x == 1 ~ "1. Agree", x == 2 ~ "2. Disagree", x == 3 ~ "3. Not
relevant", x == 4 ~ "4. Unsure", .default = "(Missing)"),
         MARITAL_STATUS = case_when(x == 1 ~ "1. Single/Never been married", x == 2 ~ "2. Married", x ==
                                         3 ~ "3. Separated", x == 4 ~ "4. Divorced", x == 5 ~ "5. Widowed", x == 6 ~ "6.
Domestic partner", .default = "(Missing)"),
         MENTAL_HEALTH = x,
         MOTHER_LOVED = case_when(x == 1 ~ "1. Yes", x == 2 ~ "2. No", x == 97 ~ "97. (Does not
apply)", .default = "(Missing)"),
         MOTHER_RELATN = case_when(x == 1 ~ "1. Very good", x == 2 ~ "2. Somewhat good", x == 3
                                      ~ "3. Somewhat bad", x == 4 ~ "4. Very bad", x == 97 ~ "97. (Does not
apply)", .default = "(Missing)"),
         NUM_CHILDREN = x,
         NUM_HOUSEHOLD = x,
         OBEY_LAW = case_when(x == 1 ~ "1. Strongly agree", x == 2 ~ "2. Somewhat agree", x == 3 ~
                                   "3. Neither agree nor disagree", x == 4 ~ "4. Somewhat disagree", x == 5 ~ "5.
Strongly disagree", .default = "(Missing)"),
         OUTSIDER = case_when(x == 1 ~ "1. Yes", x == 2 ~ "2. No", x == 97 ~ "97. (Does not apply)", .default = "(Missing)"),
         OWN_RENT_HOME = case_when(x == 1 ~ "1. Someone in this household OWNS this home", x == 2 ~
         ...
         PARENTS_12YRS = case_when(x == 1 ~ "1. Yes, married", x == 2 ~ "2. No, divorced", x == 3 ~ "3.
No, they were never married", x == 4 ~ "4. No, one or both of them had died", x
                                      == 5 ~ "5. Unsure", .default = "(Missing)"),
         PEACE = case_when(x == 1 ~ "1. Always", x == 2 ~ "2. Often", x == 3 ~ "3. Rarely", x ==
                                4 ~ "4. Never", .default = "(Missing)"),
         PEOPLE_HELP = x,
         PHYSICAL_HLTH = x,
         POLITICAL_ID = x,
         PRAY_MEDITATE = case_when(x == 1 ~ "1. More than once a day", x == 2 ~ "2. About once a day", x
                                      == 3 ~ "3. Sometimes", x == 4 ~ "4. Never", .default = "(Missing)"),
         PROMOTE_GOOD = x,
         REGION1 = case_when(x == 101 ~ "101. Argentina: Capital Federal", x == 102 ~ "102.
...
New Territories",
         ...
         REGION2 = case_when(x == 101 ~ "101. Argentina: Metropolitana", x == 102 ~ "102.
...
Vastverige",
         ...
         REGION3 = case_when(x == 1901 ~ "1901. Turkey: Aegean Region (Ege Bolgesi)", x == 1902 ~
...
Region", .default = "(Missing)"),
         ...
         REL1 = case_when(x == 1 ~ "1. Christianity", x == 2 ~ "2. Islam", x == 3 ~ "3.
...
No religion/Atheist/Agnostic",
         ...
         REL2 = case_when(x == 1 ~ "1. Christianity", x == 2 ~ "2. Islam", x == 3 ~ "3.
Hinduism", x == 4 ~ "4. Buddhism", x == 5 ~ "5. Judaism", x == 6 ~ "6.
Sikhism", x == 7 ~ "7. Baha’i", x == 8 ~ "8. Jainism", x == 9 ~ "9. Shinto",
                             x == 10 ~ "10. Taoism", x == 11 ~ "11. Confucianism", x == 12 ~ "12. Primal,
Animist, or Folk religion", x == 13 ~ "13. Spiritism", x == 14 ~ "14. Umbanda,
...
No religion/Atheist/Agnostic",
         ...
         REL3 = case_when(x == 1 ~ "1. Catholic", x == 2 ~ "2. Orthodox", x == 3 ~ "3.
...
Protestant)",
         ...
         REL4 = case_when(x == 1 ~ "1. Sunni", x == 2 ~ "2. Shi’a", x == 3 ~ "3. Sufis", x ==
...
particular (just Jewish)", .default = "(Missing)"),
         REL7 = case_when(x == 1 ~ "1. Atheist – do not believe in any god", x == 2 ~ "2.
Agnostic – unsure whether a God or gods exist", x == 3 ~ "3. Neither",
                             .default = "(Missing)"
         ),
         REL8 = case_when(x == 1 ~ "1. Spiritual", x == 2 ~ "2. Religious", x == 3 ~ "3. Both",
                             x == 4 ~ "4. Neither",
                             .default = "(Missing)"
         ),
         REL9 = case_when(x == 1 ~ "1. Jodo sect (Honen)", x == 2 ~ "2. Jodoshin sect
...
particular",
         ...
         SACRED_TEXTS = case_when(x == 1 ~ "1. More than once a day", x == 2 ~ "2. About once a day", x
                                     == 3 ~ "3. Sometimes", x == 4 ~ "4. Never", .default = "(Missing)"),
         SAT_LIVE = case_when(x == 1 ~ "1. Satisfied", x == 2 ~ "2. Dissatisfied", x == 3 ~ "3.
Unsure", .default = "(Missing)"),
         SAT_RELATNSHP = x,
         SAY_IN_GOVT = case_when(x == 1 ~ "1. Agree", x == 2 ~ "2. Disagree", x == 3 ~ "3.
Unsure", .default = "(Missing)"),
         SELFID1 = case_when(x == 101 ~ "101. Argentina: Asian", x == 102 ~ "102. Argentina:
...
(Indian, Nepalese, Pakistani)", x == 2410 ~ "2410. Hong Kong: Taiwanese", x ==
         ...
         SELFID2 = case_when(x == 101 ~ "101. Argentina: Asian", x == 102 ~ "102. Argentina:
Black", x == 103 ~ "103. Argentina: Indigenous", x == 104 ~ "104. Argentina:
Mestizo(a)", x == 105 ~ "105. Argentina: Mullato(a)", x == 106 ~ "106.
Argentina: White", x == 201 ~ "201. Australia: Aboriginal", x == 202
                                ~ "202. Australia: Australian", x == 203 ~ "203. Australia: Australian
...
(Indian, Nepalese, Pakistani)", x == 2410 ~ "2410. Hong Kong: Taiwanese", x ==
         ...
         SHOW_LOVE = x,
         SUFFERING = case_when(x == 1 ~ "1. A lot", x == 2 ~ "2. Some", x == 3 ~ "3. Not very much",
                                  x == 4 ~ "4. Not at all",
                                  .default = "(Missing)"
         ),
         SVCS_12YRS = case_when(x == 1 ~ "1. At least once a week", x == 2 ~ "2. One to three times
a month", x == 3 ~ "3. Less than once a month", x == 4 ~ "4. Never",
                                   .default =
                                     "(Missing)"
         ),
         SVCS_FATHER = case_when(x == 1 ~ "1. At least once a week", x == 2 ~ "2. One to three times
a month", x == 3 ~ "3. Less than once a month", x == 4 ~ "4. Never", x == 97 ~
                                      "97. (Does not apply)", .default = "(Missing)"),
         SVCS_MOTHER = case_when(x == 1 ~ "1. At least once a week", x == 2 ~ "2. One to three times
a month", x == 3 ~ "3. Less than once a month", x == 4 ~ "4. Never", x == 97 ~
                                      "97. (Does not apply)", .default = "(Missing)"),
         TEACHINGS_1 = x,
         TEACHINGS_2 = x,
         ...
         TEACHINGS_14 = x,
         TEACHINGS_15 = x,
         TELL_BELIEFS = case_when(x == 1 ~ "1. Agree", x == 2 ~ "2. Disagree", x == 3 ~ "3. Not
relevant", x == 4 ~ "4. Unsure", .default = "(Missing)"),
         THREAT_LIFE = case_when(x == 1 ~ "1. A lot", x == 2 ~ "2. Some", x == 3 ~ "3. Not very much",
                                    x == 4 ~ "4. Not at all",
                                    .default = "(Missing)"
         ),
         TRAITS1 = case_when(x == 1 ~ "1. Agree strongly", x == 2 ~ "2. Agree moderately", x == 3
...
strongly", .default = "(Missing)"),
         TRAITS2 = case_when(x == 1 ~ "1. Agree strongly", x == 2 ~ "2. Agree moderately", x == 3
                                ~ "3. Agree a little", x == 4 ~ "4. Neither agree nor disagree", x == 5 ~ "5.
Disagree a little", x == 6 ~ "6. Disagree moderately", x == 7 ~ "7. Disagree
strongly", .default = "(Missing)"),
         TRAITS3 = case_when(x == 1 ~ "1. Agree strongly", x == 2 ~ "2. Agree moderately", x == 3
                                ~ "3. Agree a little", x == 4 ~ "4. Neither agree nor disagree", x == 5 ~ "5.
Disagree a little", x == 6 ~ "6. Disagree moderately", x == 7 ~ "7. Disagree
strongly", .default = "(Missing)"),
         TRAITS4 = case_when(x == 1 ~ "1. Agree strongly", x == 2 ~ "2. Agree moderately", x == 3
                                ~ "3. Agree a little", x == 4 ~ "4. Neither agree nor disagree", x == 5 ~ "5.
Disagree a little", x == 6 ~ "6. Disagree moderately", x == 7 ~ "7. Disagree
strongly", .default = "(Missing)"),
         TRAITS5 = case_when(x == 1 ~ "1. Agree strongly", x == 2 ~ "2. Agree moderately", x == 3
                                ~ "3. Agree a little", x == 4 ~ "4. Neither agree nor disagree", x == 5 ~ "5.
Disagree a little", x == 6 ~ "6. Disagree moderately", x == 7 ~ "7. Disagree
strongly", .default = "(Missing)"),
         TRAITS6 = case_when(x == 1 ~ "1. Agree strongly", x == 2 ~ "2. Agree moderately", x == 3
                                ~ "3. Agree a little", x == 4 ~ "4. Neither agree nor disagree", x == 5 ~ "5.
Disagree a little", x == 6 ~ "6. Disagree moderately", x == 7 ~ "7. Disagree
strongly", .default = "(Missing)"),
         TRAITS7 = case_when(x == 1 ~ "1. Agree strongly", x == 2 ~ "2. Agree moderately", x == 3
                                ~ "3. Agree a little", x == 4 ~ "4. Neither agree nor disagree", x == 5 ~ "5.
Disagree a little", x == 6 ~ "6. Disagree moderately", x == 7 ~ "7. Disagree
strongly", .default = "(Missing)"),
         TRAITS8 = case_when(x == 1 ~ "1. Agree strongly", x == 2 ~ "2. Agree moderately", x == 3
                                ~ "3. Agree a little", x == 4 ~ "4. Neither agree nor disagree", x == 5 ~ "5.
Disagree a little", x == 6 ~ "6. Disagree moderately", x == 7 ~ "7. Disagree
strongly", .default = "(Missing)"),
         TRAITS9 = case_when(x == 1 ~ "1. Agree strongly", x == 2 ~ "2. Agree moderately", x == 3
                                ~ "3. Agree a little", x == 4 ~ "4. Neither agree nor disagree", x == 5 ~ "5.
Disagree a little", x == 6 ~ "6. Disagree moderately", x == 7 ~ "7. Disagree
strongly", .default = "(Missing)"),
         TRAITS10 = case_when(x == 1 ~ "1. Agree strongly", x == 2 ~ "2. Agree moderately", x == 3
                                 ~ "3. Agree a little", x == 4 ~ "4. Neither agree nor disagree", x == 5 ~ "5.
Disagree a little", x == 6 ~ "6. Disagree moderately", x == 7 ~ "7. Disagree
strongly", .default = "(Missing)"),
         TRUST_PEOPLE = case_when(x == 1 ~ "1. All", x == 2 ~ "2. Most", x == 3 ~ "3. Some", x == 4 ~
                                       "4. Not very many", x == 5 ~ "5. None", .default = "(Missing)"),
         URBAN_RURAL = case_when(x == 1 ~ "1. A rural area or on a farm", x == 2 ~ "2. A small town
