...
#' @export
load_meta_result <- function(dir) {
  # Use local() to limit the scope of the loaded object
  local({
    # Load the RDS file
    data <- readRDS(here::here())
    
    # Perform any necessary operations on the data here
    # For example, you might want to filter or subset the data
    # data <- subset(data, some_condition)
...
optimized_data <- load_rds_optimized(file_path)
